...
def tridiagonal_solve(A, d):
    """
    Решение трехдиагональной системы Ax = d методом прогонки.
    A - квадратная матрица размера n×n (хранится как список списков или numpy array)
    d - вектор правой части размера n
    """
    n = len(A)
    
    # Проверка размерностей
    if len(d) != n:
        raise ValueError("Размеры матрицы A и вектора d не совпадают.")
    ...
    if abs(denom) < 1e-14:
        raise ZeroDivisionError(f"Нулевой знаменатель в прогонке на последней строке.")
